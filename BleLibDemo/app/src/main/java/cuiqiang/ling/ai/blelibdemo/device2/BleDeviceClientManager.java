...
import cuiqiang.ling.ai.blelibdemo.client.BleFoundReceiver;
 ...
 * @author cuiqiang
 */
public class BleDeviceClientManager {

    private static final String TAG = "ble";
    private static BleDeviceClientManager INSTANCE;
    ...
    private BluetoothSocket mClientSocketTwo;
    private ReadThreadTwo mReadThreadTwo;
    private boolean isRegister = false;//是否注册蓝牙广播
    private boolean isShowDevice = false; // 蓝牙可见
    private boolean isWhichConnect = true;//true连接第一个,false连接第二个

    public static BleDeviceClientManager getInstance() {
        if (null == INSTANCE) {
            synchronized (BleDeviceClientManager.class) {
                if (null == INSTANCE) {
                    INSTANCE = new BleDeviceClientManager();
                }
            }
        }
        return INSTANCE;
    ...
    public void sendMessage(String msg) {
        ...
        try {
            ...
            if (null != mClientSocketTwo){
                os2 = mClientSocketTwo.getOutputStream();
     ...
     * @throws IOException
     */
    private void excuteConnectServer() throws IOException{
     ...
     * @throws IOException
     */
    private void excuteConnectServerTwo() throws IOException{
        mClientSocketTwo = mDevice.createRfcommSocketToServiceRecord(mUuid);
        String msg = mBlueToothName + mBlueToothAddress + "\n" + "客户端已启动，正在连接服务器...";
        sendHandlerMsg(SERVER_CONNECTING, msg);
        if (mClientSocketTwo != null) {
            mClientSocketTwo.connect();
        }
        String success = "已连接上服务器，可发送消息";
        sendHandlerMsg(SERVER_CONNECTED, success);
        if (mClientSocketTwo != null){
            // 启动接收数据
            mReadThreadTwo = new ReadThreadTwo(mClientSocketTwo);
    ...
    private class ReadThreadTwo extends Thread {
        ...
        public ReadThreadTwo(BluetoothSocket socket) {
            this.socket = socket;
        }
    ...
    public void setWhichConnect(boolean whichConnect) {
        isWhichConnect = whichConnect;
    }
